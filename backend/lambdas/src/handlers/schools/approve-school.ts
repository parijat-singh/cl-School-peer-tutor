...
export async function approveSchool(
  ...
  if (caller.role !== "superadmin") return error(403, "Only super admins can approve schools.");
  ...
  await ddb.send(new UpdateCommand({
    ...
    UpdateExpression: "SET approved = :true, #status = :approved",
    ExpressionAttributeNames: { "#status": "status" },
    ExpressionAttributeValues: { ":true": true, ":approved": "approved" },
  }));
