...
import { approveSchool } from "./approveSchool";
const handler = approveSchool as any;
...
describe("approveSchool", () => {
  ...
  it("rejects when school not found", async () => {
    ...
    await expect(handler({
      auth: { uid: "u1", token: { role: "superadmin" } },
      data: { domain: "notfound.edu" },
    })).rejects.toThrow("School not found");
  });

  it("approves school and writes audit log", async () => {
    mockDocGet.mockResolvedValue({ exists: true, data: () => ({ name: "Test School" }) });
    const result = await handler({
      auth: { uid: "u1", token: { role: "superadmin" } },
      data: { domain: "school.edu" },
    });
    expect(result).toEqual({ success: true });
    expect(mockDocUpdate).toHaveBeenCalledWith({ approved: true, status: "approved" });
